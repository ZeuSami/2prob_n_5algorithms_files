...
def main():
    ...
    param_activation = 'relu' #activation function
    param_num_neuron = 8 #set number of neurons on one layer
    param_layers = (param_num_neuron)#set layer structure
    ...
    # estimator = MLPClassifier(hidden_layer_sizes=param_layers, activation='relu',random_state=RANDOM_STATE, max_iter=300, solver='sgd')
    # plot_learning_curve(
    #     estimator, title, X_train, Y_train, axes=axes[:,1], ylim=(0.4, 1.01), cv=cv, n_jobs=4
    # )
    # estimator.fit(X_train,Y_train)
    # display_metrics("Network",estimator.predict(X_test),Y_test)
    
    # title = "Learning Curves NN, identity"
    # cv = cv = KFold(n_splits=param_kfold)   
    # estimator = MLPClassifier(hidden_layer_sizes=param_layers, activation='identity',random_state=RANDOM_STATE, max_iter=300, solver='sgd')
    # plot_learning_curve(
    ...
    # cv = cv = KFold(n_splits=param_kfold)   
    # estimator = MLPClassifier(hidden_layer_sizes=param_layers, activation='tanh',random_state=RANDOM_STATE, max_iter=300, solver='sgd')
    # plot_learning_curve(
    ...
    cv = cv = KFold(n_splits=param_kfold)   
    estimator = MLPClassifier(hidden_layer_sizes=param_layers, activation='logistic',random_state=RANDOM_STATE, max_iter=300, solver='sgd')
    plot_learning_curve(
